...
def generate_ideas(
    ...
    if skip_generation:
        ...
        if 1:
            if exist_idea_file:
                ...
                return ideas
            with open(osp.join(base_dir, "new_ideas_after_paper_review.json"), "r") as f:
                ideas = json.load(f)
            print("Loaded existing ideas from new_ideas_after_paper_review.json")
            return ideas
        else:
            print(1)
    ...
    # load exist paper text and convert to string
    with open(osp.join(review_dir, "paper.txt"), "r") as f:
        paper = f.read()

    # read paper review result
    import pandas as pd
    df = pd.read_csv(osp.join(review_dir, "gpt4.1-runs_ratings.csv"))
    # select first row and convert to dict
    paper_review_results = df.iloc[0].to_dict()
    # convert to string
    paper_review_results = json.dumps(paper_review_results)
        ...
        try:
            ...
            if not use_literature:
                ...
                text, msg_history = get_response_from_llm(
                    ...
                    # ),
                    idea_first_with_lit_review_also_with_paper_review_and_convert_to_proposal_prompt.format(
                        task_description=prompt["task_description"],
                        code=code,
                        prev_ideas_string=prev_ideas_string,
                        num_reflections=num_reflections,
                        lit_review_results=lit_review_results,
                        paper=paper,
                        paper_review_results=paper_review_results,
                    ),
                    client=client,
                    model=model,
                    system_message=idea_system_prompt,
            ...
            json_output['paper'] = paper
            json_output['paper_review_results'] = paper_review_results
    ...
    with open(osp.join(base_dir, "new_ideas_after_paper_review.json"), "w") as f:
