...
idea_first_with_lit_review_also_with_paper_review_and_convert_to_proposal_prompt = """{task_description}
<experiment.py>
{code}
</experiment.py>
...
Here are the exist paper(title, abstract) and review results(please learn from paper review result and generate new idea):
...
def generate_ideas(
    ...
    # load proposal review result
    with open(osp.join(review_dir, f"{args.experiment}_final_dedup_proposals_with_reviews.json"), "r") as f:
        proposal_review_results = json.load(f)

    # load original proposal
    with open(osp.join(base_dir, f"final_dedup_proposals.json"), "r") as f:
        proposals = json.load(f)

    idea_system_prompt = prompt["system"]

    assert len(proposals) == len(proposal_review_results), f"proposals: {len(proposals)}, proposal_review_results: {len(proposal_review_results)}"
    # check review match original proposal
    for proposal, review in zip(proposals, proposal_review_results):
        proposal_title = proposal['Title']
        if 'original_proposal' in review:
            original_proposal = review['original_proposal']
            assert original_proposal['Title'] == proposal_title
            review_summary = review['parsed_review']
        else:
            assert 'Title' in review, f"Title not in review, review: {review}"
            assert review['Title'] == proposal_title
            review_summary = review['review_results']['summary']

    # generate new proposal base on old proposal and review result
    kk = 0
    for proposal, review in zip(proposals, proposal_review_results):
        proposal_title = proposal['Title']
        if 'original_proposal' in review:
        ...
        print()
        print(f"Generating idea {kk + 1}/{len(proposal_review_results)}")
        kk += 1
        try:
            ...
            if not use_literature:
                ...
                # print(f"===================\n\nlit_review_results:\n{lit_review_results}\n\n===================\n\n")
                msg = idea_first_with_lit_review_also_with_proposal_review_and_convert_to_proposal_prompt.format(
                        task_description=prompt["task_description"],
                        code=code,
            ...
            data = {
                ...
                'old_proposal_review_results': review_summary,
                'new_proposal': json_output,
                'prompt':msg,
                "raw_output":text,
            }
            new_idea_str_archive.append(json.dumps(data, ensure_ascii=False))
        # else:
...
def check_idea_novelty(
    ...
    results_file = osp.join(base_dir, "new_ideas_after_paper_review.json")
