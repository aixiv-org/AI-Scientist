...
# 2. Expand the Introduction, Related Work, Method, Experiment, and Conclusion sections according to the meta review results.
def perform_improvement(review, coder):
    improvement_prompt = '''\n
## Your Role:
You are a seasoned academic researcher and an expert in AI. You have extensive experience in revising manuscripts for top-tier journals and conferences (ICLR, CVPR, ACL, ...). Your task is to help me revise my research paper based on the provided meta-review to enhance its quality, clarity, and impact.



Here is the detail Meta-Review Comments:
...
Your primary goal is to perform a comprehensive revision of my LaTeX article((template.tex)). This revision should address all the points raised in the meta-review. The final output should be a polished, publication-ready manuscript that is clear, coherent, and effectively communicates the research findings.

## Step-by-Step Instructions for Revision:

Please follow these steps methodically:

1.  Initial Analysis: First, carefully read and analyze the entire original article((template.tex)) and all the meta-review comments to understand the reviewers' concerns and suggestions.

2.  Structure the Revision Plan: Create a high-level plan for how you will address the major and minor points from the review. You can present this as a bulleted list. For example:
    Strengthen the introduction and motivation.
    Clarify the methodology in Section X.
    Revise the presentation of results in Table Y.
    Tone down the claims in the conclusion.

3.  Point-by-Point Revisions: Address each specific comment from the meta-review one by one. For each point, I want you to:
    Identify the concern: Briefly state the reviewer's comment or criticism.
    Propose a change: Describe the specific modification you will make to the text. This could involve rewriting sentences, adding new paragraphs, clarifying terminology, or adding citations.
    Implement the change: Provide the revised LaTeX code for the relevant sections.

4.  Global Revisions: After addressing the specific points, perform a global revision of the manuscript to ensure:
    Clarity and Flow: The paper reads smoothly, and the arguments are easy to follow.
    Consistency: The terminology, notation, and formatting are consistent throughout the paper.
    Tone: The tone is academic, objective, and confident, without being overly defensive or dismissive of the reviewers' feedback.

## Output Requirements:

1.  A Revised LaTeX File(template.tex): Provide the complete, revised LaTeX code for the entire article with all the changes incorporated.

## Final Check

Before providing the final output, please do a final read-through to catch any typos, grammatical errors, or formatting issues in both the revised LaTeX file(template.tex) .

Please begin the revision process..'''.format(
        review=json.dumps(review)
    )

    print("improvement_prompt:", improvement_prompt)
