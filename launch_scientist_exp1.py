...
from ai_scientist.generate_ideas_with_proposal_review import generate_ideas, check_idea_novelty
...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default="pengsong",
        choices=["pengsong", "guowei"],
        help="peaple to review.",
    )
...
if __name__ == "__main__":
    ...
    review_dir = osp.join('data', "exp1", args.review_by)
    assert os.path.exists(osp.join(review_dir, f"{args.experiment}_final_dedup_proposals_with_reviews.json")), f"review_file not exists {osp.join(review_dir, f'{args.experiment}_final_dedup_proposals_with_reviews.json')}"

    results_dir = osp.join("results_exp1", args.experiment)
    ...
    print(f"total ideas size:{len(ideas)}")
