...
from ai_scientist.perform_review_v4 import perform_review, load_paper, perform_improvement
from ai_scientist.perform_writeup import perform_writeup, generate_latex, compile_latex
...
def parse_arguments():
    ...
    parser.add_argument(
        "--old-paper-title",
        type=str,
        help="Old Paper need to improve.",
    )
    parser.add_argument(
        "--model",
        type=str,
...
def do_idea(
    ...
    example_papers_dir = args.example_papers_dir
    print("do_idea | improvement:", improvement)
    ## CREATE PROJECT FOLDER
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    # idea_name = f"{timestamp}_{idea['Name']}"
    initial_dir = osp.join(example_papers_dir, args.old_paper_title)
    destination_dir = osp.join(results_dir, args.old_paper_title)
    folder_name = destination_dir
    if not osp.exists(destination_dir):
        shutil.copytree(initial_dir, destination_dir, dirs_exist_ok=True)
    ...
    # response_letter = osp.join(destination_dir, "response_letter.txt")
    ...
    try:
        ...
        io = InputOutput(
            yes=True, chat_history_file=f"{folder_name}/aider.txt"
        )
        # latex paper + review => new paper + response l
        ...
        #         # notes:记录的实验结果，以及plot的结果
                # fnames = [exp_file, writeup_file, notes, response_letter]
        ...
        #             return False
            ...
            if writeup == "latex" and improvement:
                ...
                try:
                    ...
                    perform_improvement(meta_review_result, coder)
                    generate_latex(
                        coder, destination_dir, f"{destination_dir}/{args.old_paper_title}_improved_v4_0727.pdf"
                    )
                    ...
                    # compile_latex(cwd, pdf_file=f"{args.old_paper_title}_improved_v4_0727_v2.pdf", timeout=timeout)
...
if __name__ == "__main__":
    ...
    # review_dir = osp.join('example_papers', args.old_paper_title)
    # assert os.path.exists(osp.join('example_papers', "ai_sci_deepreview_results.json"))

    meta_review_result = load_json_from_file(osp.join("/root/huxiang/work/AI-Scientist/data/exp2_v3_review_data", f"{args.old_paper_title}_review.json"))[0]['parsed_review']
    print("meta_review_result.keys():", meta_review_result.keys())

    results_dir = "results_exp2_v4"
    ...
    if not args.skip_run_experiment:
        if args.parallel > 0:
            ...
            # for idea in novel_ideas:
            print(f"Processing paper: {args.old_paper_title}")
            try:
                success = do_idea(
                    args,
                    results_dir,
                ...
                print(f"Completed paper: {args.old_paper_title}, Success: {success}")
            # else:
            except Exception as e:
                print(f"Failed to process paper {args.old_paper_title}: {str(e)}")
                import traceback
